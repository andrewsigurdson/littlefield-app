...
const LittlefieldAnalysis = () => {
  ...
  // Lot-based projection with job flow data for TEST settings
  ...
  const testLotProjectionSummary = useMemo(() => {
    ...
    // Debug: Show first 10 days of revenue and completions
    console.log('[SUMMARY] Lot-based projection data:', {
      totalDays: testLotProjectionData.length,
      first10Days: testLotProjectionData.slice(0, 10).map(d => ({
        day: d.day,
        revenue: d.revenue,
        completions: d.jobsCompleting,
        waitingForKits: d.jobsWaitingForKits
      })),
      dailyRevenueAvg: dailyRevenue.toFixed(3) + 'k',
      totalRevenue: totalRevenue.toFixed(2) + 'k'
    });
    ...
    <div className="w-full max-w-7xl mx-auto p-6 bg-gray-50">
      ...
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-lg shadow-lg">
          ...
          <AsyncChart
            height={380}
            title="Expected Job Flow - Test Settings"
            delay={100}
          >
            <JobFlowChart
              data={testLotProjectionData}
              title={`Expected Job Flow - Test Settings (Day ${recommendations.analysis.currentDay + 1} → 318)`}
              height={320}
            />
          </AsyncChart>

          {/* Lots by Station Chart - 3 Physical Stations */}
          <AsyncChart
            height={380}
            title="Lots by Station - Test Settings"
            delay={200}
          >
            <JobsByStationChart
              data={testLotProjectionData.map(d => ({
                day: d.day,
                lotsAtS1: (d.lotsWaitingS1 || 0) + (d.lotsProcessingS1 || 0),
                lotsAtS2: (d.lotsWaitingS2 || 0) + (d.lotsProcessingS2 || 0) + (d.lotsWaitingS4 || 0),
                lotsAtS3: (d.lotsWaitingS3 || 0) + (d.lotsProcessingS3 || 0)
              }))}
              title={`Lots by Station - Test Settings (Day ${recommendations.analysis.currentDay + 1} → 318)`}
              height={320}
            />
          </AsyncChart>
          ...
          <AsyncChart
            height={380}
            title="Projected Inventory Under Test Settings"
            delay={300}
          >
            <div className="bg-white p-6 rounded-lg shadow-md">
              <h3 className="text-xl font-bold mb-4 text-gray-800">
          ...
          <div className="bg-purple-50 p-6 rounded-lg shadow-lg border-2 border-purple-500">
            ...
            {testProjection && currentProjection && (
              <>
                <div className="bg-white p-4 rounded-lg border-2 border-purple-400">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-green-600">${formatNumber(testLotProjectionSummary.totalProfit, 2)}k</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs text-gray-600">Gross Revenue</p>
                      <p className="text-xl font-bold text-blue-600">${formatNumber(testLotProjectionSummary.grossRevenue, 0)}k</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs text-gray-600">Material Costs</p>
                      <p className="text-xl font-bold text-red-600">${formatNumber(testLotProjectionSummary.totalMaterialCost, 2)}k</p>
                    </div>
                    <div className="text-center">
                ...
                <AsyncChart
                  height={400}
                  title="Projected Station Utilization"
                  delay={800}
                >
                  <div className="bg-white p-6 rounded-lg border-2 border-purple-400 mt-4">
                ...
                <AsyncChart
                  height={400}
                  title="Projected WIP - Jobs Waiting for Kits"
                  delay={1000}
                >
                  <div className="bg-white p-6 rounded-lg border-2 border-purple-400 mt-4">
          ...
          {utilizationData.length > 0 && (
            <>
              ...
              <AsyncChart
                height={360}
                title="Historical Job Flow (Last 30 Days)"
                delay={1300}
              >
                <JobFlowChart
                  data={parsedData.slice(-30).map((d, i, arr) => {
                    // Calculate arrivals as accepted + change in queue
                    const prevQueued = i > 0 ? arr[i - 1].queuedJobs : d.queuedJobs;
                    const queueChange = d.queuedJobs - prevQueued;
                    const arrivals = d.jobsAccepted + queueChange;
              ...
              <AsyncChart
                height={360}
                title="WIP - Jobs Waiting for Kits (Last 30 Days)"
                delay={1600}
              >
                <div className="bg-white p-6 rounded-lg shadow-md">
